...
class Safemode(BASE):
    ...
    strong_status = Column(Boolean, default=False)

    def __init__(self, chat_id, safemode_status=False, strong_status=False):
        self.chat_id = str(chat_id) # ensure string
        self.safemode_status = safemode_status
        self.strong_status = strong_status
...
def strong_safemode(chat_id, strong_status=True):
    with SAFEMODE_INSERTION_LOCK:
        curr = SESSION.query(Safemode).get((str(chat_id)))
        if curr:
            SESSION.delete(curr)
        switch_status = Safemode(str(chat_id), strong_status)
